...
const NavItem = ({title, href, active}: { title: string, href: string, active: boolean }) => {
    return (
        <Link href={href}
              className={classNames(active ? 'bg-white' : 'bg-green-700', 'text-black w-72 text-3xl text-left px-3.5 py-3')}>
            {title}
        </Link>
    )
}
                    ...
                    <div className={"flex flex-col gap-10 items-end"}>
                        {
                            navLinks.map((link) => {
                                return (
                                    <NavItem href={link.href} active={activePage === link.title} key={link.title}
                                             title={link.title}/>
                                )
                            })
                        }
